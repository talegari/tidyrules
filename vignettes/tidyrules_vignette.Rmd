...
abstract: The package [`tidyRules`](https://cran.r-project.org/package=tidyrules) is meant to extract rules from model objects to a clean `tibble` format. Currently this package supports `C5.0`, `rpart` and `cubist` models. 
---

## Overview
This vignette would guide you through tidyrules usage with some toy examples. 

As a matter of fact, rules from different algorithms like [C5.0](https://cran.r-project.org/web/packages/C50/index.html), [cubist](https://cran.r-project.org/web/packages/Cubist/index.html) and [rpart](https://cran.r-project.org/web/packages/rpart/index.html)
are readable but, hard to use respective rules for further analysis. 
[**tidyrules**](https://cran.r-project.org/package=tidyrules) extract rules from 
the model objects to a clean and tidy `tibble`. One can 
subject those tidy rules for furhter deep-dive using `dplyr` functions. 
...
__`tidyRules` important columns to notice :__
  ...
  * `confidence` : Prediction accuracy for respective class. 
...
illustration shows how to extract rules fom `C5.0` model  and applying `filter()` 
...
dataset. Here our aim is to predict Attrition using rest of the variables. Let 
...
Whatif we have all the rules in a tidy table format so that we could easily use 
...
look at rules which favours only Attrition is equal to "No" and arrange by 
support.
...
*__NOTE__ : Do not forget to convert all `ordered` featrues to `factor` type
before trainig the model.*

One could visualise rpart descision tree using `prp` function from `rpart.plot` package. 
```{r,warning=FALSE,echo=TRUE,message=FALSE}
library("rpart.plot")
prp(rpart_bc)
```

The above tree visual is really nice to get a hang of how desision tree is splitting at each node. But, if you want to pick a terminal node it is really boring and hard 
since one has to enter the respective filter manually (imagine a cituation if you have hundreds of features and a huge tree!!). To get-ride of this problem
